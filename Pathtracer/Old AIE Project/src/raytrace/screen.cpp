...
colour *gcolours = nullptr;
unsigned gwidth = 0, gheight = 0, gwidthheight = 0;

void initialiseScreen(unsigned width, unsigned height)
{
	gcolours = new colour[gwidthheight = (gwidth = width) * (gheight = height)];
}

void setScreenColour(unsigned x, unsigned y, const colour &colour_, bool andIllumination)
{
	int ind = min(x, gwidth - 1) + min(y, gheight - 1) * gwidth;
	if (andIllumination)
		gcolours[ind] = colour_;
	else
	{
		colour &col = gcolours[ind];
		col.r = colour_.r;
		col.g = colour_.g;
		col.b = colour_.b;
	}
...
const colour& getScreenColour(unsigned x, unsigned y)
{
	return gcolours[min(x, gwidth - 1) + min(y, gheight - 1) * gwidth];
}

uvec2 getScreenDims()
{
	return uvec2(gwidth, gheight);
}

void finaliseScreen(const char* output)
{
	wchar_t path[512];    // don't really use a fixed size buffer!
	std::wcout << output;
	std::wcin >> path;


	BYTE* buf = new BYTE[gwidthheight * 3];
	int c = 0;
	...
	for (int i = 0; i < gheight; i++)
	{
		...
		{
			colour &col = gcolours[j + i * gwidth];

			buf[c + 0] = (BYTE)(col.b * 255);
			buf[c + 1] = (BYTE)(col.g * 255);
			buf[c + 2] = (BYTE)(col.r * 255);
