...
void Gizmos::addFrustum(const glm::vec3& a_position, const glm::vec3& a_lookVector, float a_fov, float a_aspect, float a_near, float a_far, const glm::vec4& a_colour, const glm::mat4* a_transform)
{
	//TODO: a_transform not used.
	glm::mat4 frustumMat = glm::perspective(a_fov, a_aspect, a_near, a_far) * glm::lookAt(a_position, a_position + a_lookVector, glm::vec3(0.0F, 1.0F, 0.0F));
	// invert the matrix
	frustumMat = glm::inverse(frustumMat);
	...
	ntl = frustumMat * glm::vec4(-1.0F, 1.0F, n, 1.0F);
	ntr = frustumMat * glm::vec4(1.0F, 1.0F, n, 1.0F);
	nbl = frustumMat * glm::vec4(-1.0F, -1.0F, n, 1.0F);
	nbr = frustumMat * glm::vec4(1.0F, -1.0F, n, 1.0F);

	ftl = frustumMat * glm::vec4(-1.0F, 1.0F, f, 1.0F);
	ftr = frustumMat * glm::vec4(1.0F, 1.0F, f, 1.0F);
	fbl = frustumMat * glm::vec4(-1.0F, -1.0F, f, 1.0F);
	fbr = frustumMat * glm::vec4(1.0F, -1.0F, f, 1.0F);
	...
	glm::vec4 farCol = a_colour * glm::vec4(0.2F, 0.2F, 0.2F, 0.1F);

	addLine(ntl.xyz, ntr.xyz, a_colour);
	addLine(nbl.xyz, nbr.xyz, a_colour);
	addLine(ntl.xyz, nbl.xyz, a_colour);
	addLine(ntr.xyz, nbr.xyz, a_colour);

	addLine(ftl.xyz, ftr.xyz, farCol);
	addLine(fbl.xyz, fbr.xyz, farCol);
	addLine(ftl.xyz, fbl.xyz, farCol);
	addLine(ftr.xyz, fbr.xyz, farCol);

	addLine(a_position, ftl.xyz, a_colour, farCol);
	addLine(a_position, fbl.xyz, a_colour, farCol);
	addLine(a_position, fbr.xyz, a_colour, farCol);
	addLine(a_position, ftr.xyz, a_colour, farCol);
}
