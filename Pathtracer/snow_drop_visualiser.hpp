...
{
public:
	SDV_API sdvContext(const char *name, unsigned width, unsigned height);
	SDV_API ~sdvContext();
	SDV_API void hookInit(sdv_fptr_NoArg callback);
	SDV_API void hookUpdate(sdv_fptr_CamVecs4DT callback);
	SDV_API void hookStop(sdv_fptr_NoArg callback);
	SDV_API void start();
};
